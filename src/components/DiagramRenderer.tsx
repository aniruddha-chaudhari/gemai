...
const MarkmapDiagram = memo(({ code }: { code: string }) => {
  ...
  const openInFullPage = () => {
    const encodedCode = encodeURIComponent(code)
    window.location.href = `/diagram/markmap?code=${encodedCode}`
  }
...
MarkmapDiagram.displayName = 'MarkmapDiagram'

export function DiagramRenderer({ code, type, language, onError }: DiagramRendererProps) {
  if (type === 'mermaid') {
    return <MermaidDiagram code={code} onError={onError} />
  } else if (type === 'markmap') {
    return <MarkmapDiagram code={code} />
  }
  return null
}
